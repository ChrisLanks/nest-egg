...
class TestCalculateMonthlyPayment:
    def test_known_loan(self):
        ...
        payment = svc.calculate_monthly_payment(Decimal("200000"), Decimal("6.0"), 360)
        assert Decimal("1199.00") <= payment <= Decimal("1200.00")

    def test_zero_interest(self):
        """0% rate: simple division principal / months."""
        payment = svc.calculate_monthly_payment(Decimal("12000"), Decimal("0"), 12)
    ...
    def test_short_term_high_rate(self):
        ...
        payment = svc.calculate_monthly_payment(Decimal("1000"), Decimal("24"), 12)
...
class TestCalculatePayoffMonths:
    def test_zero_interest_simple_division(self):
        months = svc.calculate_payoff_months(Decimal("1200"), Decimal("0"), Decimal("100"))
        assert months == 12

    def test_payment_less_than_interest(self):
        """$10K at 24%, min $10 → interest ~$200/mo, never pays off."""
        months = svc.calculate_payoff_months(Decimal("10000"), Decimal("24"), Decimal("10"))
        assert months == 999

    def test_zero_payment(self):
        months = svc.calculate_payoff_months(Decimal("1000"), Decimal("10"), Decimal("0"))
        assert months == 999

    def test_zero_balance(self):
        months = svc.calculate_payoff_months(Decimal("0"), Decimal("10"), Decimal("100"))
        assert months == 0

    def test_known_payoff(self):
        """$5000 at 18%, $200/mo — should be roughly 31 months."""
        months = svc.calculate_payoff_months(Decimal("5000"), Decimal("18"), Decimal("200"))
        assert 28 <= months <= 34
...
class TestTotalInterest:
    def test_zero_interest_rate(self):
        ...
        interest = svc.calculate_total_interest(Decimal("12000"), Decimal("0"), Decimal("1000"))
        assert interest == Decimal("0.00")

    def test_impossible_payoff(self):
        """Payment too low → sentinel 999999."""
        interest = svc.calculate_total_interest(Decimal("100000"), Decimal("24"), Decimal("10"))
        assert interest == Decimal("999999")

    def test_known_interest(self):
        """$10K at 6% paid $200/mo → total interest should be reasonable."""
        interest = svc.calculate_total_interest(Decimal("10000"), Decimal("6"), Decimal("200"))
