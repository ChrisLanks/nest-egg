...
Revision ID: a1b2c3d4e5f6_add_missing_columns_ocr_guest_report
...
revision: str = "a1b2c3d4e5f6_add_missing_columns_ocr_guest_report"
...
def upgrade() -> None:
    # transaction_attachments: OCR columns
    op.add_column(
    ...
    # report_templates: scheduled_delivery column
    op.add_column(
        "report_templates",
        sa.Column("scheduled_delivery", sa.JSON(), nullable=True),
    )

    # household_guests: optional auto-revoke date
    op.add_column(
        "household_guests",
        sa.Column("expires_at", sa.DateTime(), nullable=True),
    )

    # household_guest_invitations: days until guest access auto-revokes after accepting
    op.add_column(
