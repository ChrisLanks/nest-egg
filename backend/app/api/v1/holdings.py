...
async def get_portfolio_summary(
    ...
    investment_account_ids = [acc.id for acc in accounts if acc.account_type in investment_account_types]
    ...
    if investment_account_ids:
        result = await db.execute(
            ...
            .join(Account)
            .where(
                Holding.account_id.in_(investment_account_ids)
            )
...
async def get_rmd_summary(
    ...
    if user_id:
        ...
        accounts = [acc for acc in accounts if acc.account_type in rmd_account_types and acc.is_active]
    ...
    # Combined household view
        ...
        for member in household_members:
            ...
            # Filter for RMD-applicable accounts
            member_accounts = [acc for acc in member_accounts if acc.account_type in rmd_account_types and acc.is_active]
