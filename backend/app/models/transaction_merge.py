...
class TransactionMerge(Base):
    ...
    primary_transaction_id = Column(UUID(as_uuid=True), ForeignKey("transactions.id", ondelete="CASCADE"), nullable=False, index=True)

    # Duplicate transaction (the one that was merged/deleted)
    duplicate_transaction_id = Column(UUID(as_uuid=True), nullable=False, index=True)  # No FK since it's deleted
    ...
    merged_by_user_id = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
