...
def validate_symbol(symbol: str) -> str:
    ...
    for pattern in DANGEROUS_PATTERNS:
        if pattern in symbol:
            raise SymbolValidationError(
                f"Symbol contains forbidden pattern: {pattern}"
            )
...
def validate_price(price: Decimal, symbol: str = "") -> Decimal:
    ...
    if price > Decimal("10000000"):  # $10M per share
        ...
        raise PriceValidationError(
            f"Price suspiciously high for {symbol}: ${price}"
        )
...
def validate_quote_response(raw_data: dict, symbol: str) -> ValidatedQuoteData:
    ...
    try:
        ...
        logger.info(
            f"Successfully validated quote for {validated.symbol}: "
            f"${validated.price}"
        )
        ...
        logger.error(
            f"Quote validation failed for {symbol}: {e}",
            exc_info=True
        )
        raise PriceValidationError(
            f"Invalid quote data for {symbol}: {str(e)}"
        )
