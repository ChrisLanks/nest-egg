...
class ErrorLoggingService:
    ...
    def redact_pii(text: str) -> str:
        ...
        # Redact emails
        redacted = re.sub(ErrorLoggingService.PII_PATTERNS["email"], "[REDACTED_EMAIL]", redacted)

        # Redact SSNs
        redacted = re.sub(ErrorLoggingService.PII_PATTERNS["ssn"], "[REDACTED_SSN]", redacted)

        # Redact credit cards
        redacted = re.sub(
            ErrorLoggingService.PII_PATTERNS["credit_card"], "[REDACTED_CC]", redacted
        )

        # Redact phone numbers
        redacted = re.sub(ErrorLoggingService.PII_PATTERNS["phone"], "[REDACTED_PHONE]", redacted)
    ...
    def log_error(
        ...
        if context:
            ...
            safe_context = {k: ErrorLoggingService.redact_pii(str(v)) for k, v in context.items()}
    ...
    def log_security_event(
        ...
        if additional_data:
            log_data["data"] = {
                k: ErrorLoggingService.redact_pii(str(v)) for k, v in additional_data.items()
            }
    ...
    def sanitize_request_data(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            ...
            if any(
                ...
                sanitized[key] = [
                    (
                        ErrorLoggingService.sanitize_request_data(item)
                        if isinstance(item, dict)
                        else (
                            ErrorLoggingService.redact_pii(str(item))
