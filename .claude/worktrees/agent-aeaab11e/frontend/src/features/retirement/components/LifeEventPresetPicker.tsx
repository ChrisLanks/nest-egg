...
export function LifeEventPresetPicker({
  ...
  const formatCost = (preset: LifeEventPreset) => {
    if (preset.annual_cost)
      return `$${(preset.annual_cost / 1000).toFixed(0)}K/yr`;
    if (preset.one_time_cost)
      return `$${(preset.one_time_cost / 1000).toFixed(0)}K`;
                    ...
                    {categoryPresets.map((preset) => (
                      ...
                      >
                        <VStack align="start" spacing={0}>
                          ...
                          <Text
                            fontSize="xs"
                            color={`${CATEGORY_COLORS[category] || "gray"}.500`}
                          >
                            {formatCost(preset)}
                            {preset.duration_years
                              ? ` for ${preset.duration_years}yr`
                              : ""}
                          </Text>
                        </VStack>
                      </Button>
