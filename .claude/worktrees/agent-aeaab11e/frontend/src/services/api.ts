...
// Guest household header injection — registered by householdStore to avoid circular imports.
// The store calls registerHouseholdGetter() on init; until then, no header is sent.
let _getActiveHouseholdId: (() => string | null) | null = null;
export const registerHouseholdGetter = (getter: () => string | null) => {
...
api.interceptors.request.use(
  async (config) => {
    ...
    // Inject guest household header when viewing another household
    if (_getActiveHouseholdId) {
    ...
    if (
      ...
      try {
        ...
        devError(
          ...
          refreshError.response?.data?.detail || refreshError.message,
        );
        useAuthStore.getState().logout();
        window.location.href = "/login";
