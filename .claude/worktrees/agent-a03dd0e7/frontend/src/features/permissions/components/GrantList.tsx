...
export const GrantList = ({ grants, mode }: GrantListProps) => {
  ...
  const [revokeTarget, setRevokeTarget] = useState<PermissionGrant | null>(
    null,
  );
  const {
    isOpen: isEditOpen,
          ...
          {grants.map((g) => {
            ...
            return (
              <Tr key={g.id} opacity={isExpired ? 0.5 : 1}>
                ...
                <Td>
                  {RESOURCE_TYPE_LABELS[g.resource_type] ?? g.resource_type}
                </Td>
                <Td>
                  <HStack spacing={1} wrap="wrap">
                    {g.actions
                      ...
                      .map((a) => (
                        <Badge
                          key={a}
                          colorScheme={ACTION_COLORS[a] ?? "gray"}
                          size="sm"
                        >
                          {a}
                        </Badge>
        ...
        <AlertDialogOverlay>
          <AlertDialogContent>
            ...
            <AlertDialogBody>
              ...
              <strong>
                {revokeTarget?.grantee_display_name ?? "this user"}
              </strong>
              .
            </AlertDialogBody>
            <AlertDialogFooter>
