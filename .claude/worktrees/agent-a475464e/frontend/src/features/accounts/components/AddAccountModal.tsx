...
export const AddAccountModal = ({ isOpen, onClose }: AddAccountModalProps) => {
  ...
  const [selectedProvider, setSelectedProvider] = useState<BankProvider | null>(
    null,
  );
  ...
  const handlePrivateEquityAccountSubmit = (
    data: PrivateEquityAccountFormData,
  ) => {
    createManualAccountMutation.mutate(data);
  };
    ...
    switch (currentStep) {
      ...
      case "manual_form_bond":
        return selectedAccountType
          ? `Add ${formatAccountType(selectedAccountType)} Account`
          : "Account Details";
      case "bank_link": {
        const providerName =
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} size="2xl">
      ...
      <ModalContent>
        ...
        <ModalBody pb={6}>
          <FormErrorBoundary>
            {/* Step 1: Source Selection (auto-selects if only one provider enabled) */}
            {currentStep === "source_selection" && (
              <SourceSelectionStep
                onSelectSource={handleSelectSource}
                skipAutoSelect={userNavigatedBack}
              />
            )}
            ...
            {currentStep === "manual_form_investment" &&
              selectedAccountType && (
            ...
            {currentStep === "manual_form_private_equity" && (
              <PrivateEquityAccountForm
                ...
                isLoading={createManualAccountMutation.isPending}
                defaultAccountType={
                  selectedAccountType === "stock_options"
                    ? "stock_options"
                    : "private_equity"
                }
              />
