...
print_next_steps() {
    ...
    echo "   Frontend:      ${GREEN}http://localhost:3000${NC} (Docker) or ${GREEN}http://localhost:5173${NC} (local)"
    ...
    echo "   ${YELLOW}README.Docker.md${NC}  # Quick Docker reference"
    echo "   ${YELLOW}DEPLOYMENT.md${NC}     # Full deployment guide"
    echo ""

    echo -e "${YELLOW}⚠ Important:${NC} Configure account provider credentials in ${YELLOW}.env${NC}:"
    echo ""
    echo "   ${YELLOW}Plaid${NC} (11,000+ institutions, ~\$0.50-\$2/account/month):"
    echo "   PLAID_CLIENT_ID=your_client_id"
    echo "   PLAID_SECRET=your_secret"
    echo ""
    echo "   ${YELLOW}Teller${NC} (⭐ Recommended - 100 FREE accounts/month!):"
    echo "   TELLER_APP_ID=your_teller_app_id"
    echo "   TELLER_API_KEY=your_teller_api_key"
    echo ""
    echo "   ${GREEN}Sign up:${NC} Plaid: https://dashboard.plaid.com/signup"
