...
export const savingsGoalsApi = {
  ...
  update: async (
    ...
    const { data } = await api.patch<SavingsGoal>(
  ...
  fund: async (
    goalId: string,
    method: "waterfall" | "proportional" = "waterfall",
  ): Promise<SavingsGoal> => {
    const { data } = await api.post<SavingsGoal>(
      `/savings-goals/${goalId}/fund`,
      { method },
    );
    return data;
  },
  ...
  autoSync: async (
    ...
    const { data } = await api.post<SavingsGoal[]>("/savings-goals/auto-sync", {
      method,
    });
    return data;
  },
