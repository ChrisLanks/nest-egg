...
const DebtSummaryWidgetBase: React.FC = () => {
  ...
  const debtAccounts = (data?.account_balances ?? []).filter(
    (a: { type: string; balance: number }) =>
      DEBT_TYPES.has(a.type) && a.balance < 0,
  );
          ...
          {debtAccounts.slice(0, 5).map(
            (
              ...
              <Box key={account.id}>
                ...
                {index < debtAccounts.slice(0, 5).length - 1 && (
                  <Divider mt={2} />
                )}
              </Box>
            ),
...
export const DebtSummaryWidget = memo(DebtSummaryWidgetBase);
