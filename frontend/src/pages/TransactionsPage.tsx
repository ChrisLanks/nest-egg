...
interface TransactionDesktopRowProps {
  ...
  onToggleSelection: (id: number, e: any, index: number) => void;
...
const TransactionDesktopRow = memo(
  ({
    txn,
    globalIndex,
    isSelected,
    ...
    >
      <Td width="40px" onClick={(e) => e.stopPropagation()}>
        <Checkbox
          ...
          onChange={(e) =>
            onToggleSelection(txn.id, e.nativeEvent as any, globalIndex)
          }
        />
      </Td>
      ...
      {showAccountColumn && (
        <Td
          onClick={(e) =>
            txn.account_name && onAccountClick(txn.account_name, e)
          }
        >
          <Text
            fontSize="sm"
      ...
      {showCategoryColumn && (
        <Td
          onClick={(e) => {
            ...
            if (categoryName) onCategoryClick(categoryName, e);
          }}
        >
            ...
            {txn.labels?.map((label) => (
              <WrapItem key={label.id}>
                <Badge
                  ...
                  _hover={{ transform: "scale(1.05)" }}
                  transition="transform 0.2s"
                  onClick={(e) => onLabelClick(label.name, e)}
                >
                  {label.name}
                </Badge>
              </WrapItem>
...
const TransactionMobileCard = memo(
  ({
    txn,
    globalIndex,
    isSelected,
            ...
            {txn.account_name && (
              <HStack justify="space-between">
                ...
                <Text
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    onAccountClick(txn.account_name!, e);
                  }}
                >
                  {txn.account_name}
                  {txn.account_mask && ` ****${txn.account_mask}`}
              ...
              <HStack justify="space-between">
                ...
                <Badge
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    const categoryName =
                      txn.category?.name || txn.category_primary;
                    if (categoryName) onCategoryClick(categoryName, e);
                  }}
                >
                  {txn.category
...
export const TransactionsPage = () => {
  ...
  const [selectedLabelToRemove, setSelectedLabelToRemove] =
    useState<string>("");
  ...
  const bulkMarkTransferMutation = useMutation({
    mutationFn: async ({
      ...
      return {
        attempted: transactionIds.length,
    ...
    onError: (error: any) => {
      const message =
        error.message === "No owned transactions to modify"
  ...
  const handleAddLabelToPending = () => {
    if (
      selectedLabelToAdd &&
      !pendingLabelsToAdd.includes(selectedLabelToAdd)
    ) {
  ...
  const handleAddLabelToRemovePending = () => {
    if (
      selectedLabelToRemove &&
      !pendingLabelsToRemove.includes(selectedLabelToRemove)
    ) {
      setPendingLabelsToRemove([
        ...pendingLabelsToRemove,
        selectedLabelToRemove,
      ]);
  ...
  const handleApplyBulkLabelChanges = async () => {
    ...
    const labelsToAdd = pendingLabelsToAdd.filter(
      (id) => !pendingLabelsToRemove.includes(id),
    );
    const labelsToRemove = pendingLabelsToRemove.filter(
      (id) => !pendingLabelsToAdd.includes(id),
    );
  ...
  const handleCategoryClick = (category: string, e: React.MouseEvent) => {
    ...
    const hasQuotedVersion =
      searchQuery.includes(`categories:${quotedCategory}`) ||
      searchQuery.includes(`category:${quotedCategory}`);
    const hasUnquotedVersion =
  ...
  const handleLabelClick = (labelName: string, e: React.MouseEvent) => {
    ...
    const hasQuotedVersion =
      searchQuery.includes(`labels:${quotedLabel}`) ||
      searchQuery.includes(`label:${quotedLabel}`);
    const hasUnquotedVersion =
      searchQuery.includes(`labels:${labelName}`) ||
      searchQuery.includes(`label:${labelName}`);
    ...
    const formattedLabel =
      labelName.includes(" ") || labelName.includes(",")
        ? quotedLabel
        : labelName;
  ...
  const handleAccountClick = (accountName: string, e: React.MouseEvent) => {
    ...
    const hasQuotedVersion =
      searchQuery.includes(`accounts:${quotedAccount}`) ||
      searchQuery.includes(`account:${quotedAccount}`);
    const hasUnquotedVersion =
      searchQuery.includes(`accounts:${accountName}`) ||
      searchQuery.includes(`account:${accountName}`);
    ...
    const formattedAccount =
      accountName.includes(" ") || accountName.includes(",")
        ? quotedAccount
        : accountName;
  ...
  const handleBulkCreateRule = () => {
    ...
    const categoryIds = new Set(
      selected.map((t) => t.category?.id).filter(Boolean),
    );
    ...
    <Container maxW="container.xl" py={8}>
      <VStack spacing={6} align="stretch">
          ...
          >
            <HStack>
              ...
              <Text fontSize="sm" color="orange.800" fontWeight="medium">
                Read-only view: You can view transactions but cannot bulk select
                or create rules for another household member.
              </Text>
            </HStack>
          </Box>
        )}
                ...
                <Tr>
                  ...
                  {showDateColumn && (
                    ...
                    >
                      <HStack spacing={1}>
                        <Text>Date</Text>
                        <SortIcon {...sortIconProps} field="date" />
                      </HStack>
                    </Th>
                  )}
                  {showMerchantColumn && (
                    ...
                    >
                      <HStack spacing={1}>
                        <Text>Merchant</Text>
                        <SortIcon {...sortIconProps} field="merchant_name" />
                      </HStack>
                    </Th>
                  )}
                  {showAccountColumn && (
                    ...
                    >
                      <HStack spacing={1}>
                        <Text>Account</Text>
                        <SortIcon {...sortIconProps} field="account_name" />
                      </HStack>
                    </Th>
                  )}
                  {showCategoryColumn && (
                    ...
                    >
                      <HStack spacing={1}>
                        <Text>Category</Text>
                        <SortIcon {...sortIconProps} field="category_primary" />
                      </HStack>
                    </Th>
                  )}
                  {showLabelsColumn && (
                    ...
                    >
                      <HStack spacing={1}>
                        <Text>Labels</Text>
                        <SortIcon {...sortIconProps} field="labels" />
                      </HStack>
                    </Th>
                  )}
                  {showAmountColumn && (
                    ...
                    >
                      <HStack spacing={1} justify="flex-end">
                        <Text>Amount</Text>
                        <SortIcon {...sortIconProps} field="amount" />
                      </HStack>
                    </Th>
                  )}
                  {showStatusColumn && (
                    ...
                    >
                      <HStack spacing={1}>
                        <Text>Status</Text>
                        <SortIcon {...sortIconProps} field="status" />
                      </HStack>
                    </Th>
                  )}
                </Tr>
                  ...
                  return (
                    <React.Fragment key={monthGroup.period}>
                      ...
                      {monthGroup.transactions.map((txn, localIndex) => {
                        ...
                        const { formatted, isNegative } = formatCurrency(
                          txn.amount,
                        );
                ...
                {bulkActionType === "mark" ? (
                  <>
                    <Text>
                      ...
                      <strong>
                        {selectedTransactions.size} transaction(s)
                      </strong>{" "}
                      as transfers?
                    </Text>
                    <Text mt={2} color="orange.600" fontSize="sm">
                      Transfers are excluded from cash flow calculations and
                      budgets. This action affects how your financial reports
                      are calculated.
                    </Text>
                  </>
                ) : (
                  <>
                    ...
                    <Text mt={2} color="blue.600" fontSize="sm">
                      These transactions will be included in cash flow
                      calculations and budgets again.
                    </Text>
                  </>
                )}
              </AlertDialogBody>
        ...
        <Modal isOpen={isBulkEditOpen} onClose={handleBulkEditClose} size="xl">
          ...
          <ModalContent>
            <ModalHeader>
              Bulk Edit {selectedTransactions.size} Transaction(s)
            </ModalHeader>
            <ModalCloseButton />
            <ModalBody>
              <Tabs>
                      ...
                      <FormControl>
                        ...
                        <HStack>
                          <Select
                            ...
                            onChange={(e) =>
                              setSelectedLabelToAdd(e.target.value)
                            }
                          >
                            {availableLabels
                              .filter(
                                ...
                                <WrapItem key={labelId}>
                                  ...
                                  >
                                    ...
                                    <CloseButton
                                      size="sm"
                                      onClick={() =>
                                        handleRemoveLabelFromAddPending(labelId)
                                      }
                                    />
                                  </Badge>
                                </WrapItem>
                      ...
                      <FormControl>
                        ...
                        <HStack>
                          <Select
                            ...
                            onChange={(e) =>
                              setSelectedLabelToRemove(e.target.value)
                            }
                          >
                            {availableLabels
                              .filter(
                                ...
                                <WrapItem key={labelId}>
                                  ...
                                  >
                                    ...
                                    <CloseButton
                                      size="sm"
                                      onClick={() =>
                                        handleRemoveLabelFromRemovePending(
                                          labelId,
                                        )
                                      }
                                    />
              ...
              pendingLabelsToRemove.length > 0 ? (
                <>
                  <Button
                    ...
                    isLoading={
                      bulkAddLabelMutation.isPending ||
                      bulkRemoveLabelMutation.isPending
                    }
                  >
                    Apply Label Changes
                  </Button>
