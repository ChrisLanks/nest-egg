...
export const AccountsPage = () => {
  ...
  // Client-side filter for partial multi-select in combined view
  ...
  }, [selectedUserId, selectedIdsKey, queryClient]);
  ...
  const toggleInstitutionVisibility = (accounts: Account[]) => {
    const modifiableAccounts = accounts.filter((account) =>
  ...
  const handleDeleteConfirm = () => {
    if (deleteTarget === "selected") {
      ...
      const ownedAccountIds = Array.from(selectedAccounts).filter(
        (accountId) => {
          const account = accounts?.find((a) => a.id === accountId);
          return account && canModifyAccount(account);
        },
      ...
      <VStack spacing={6} align="stretch">
        ...
        {Object.entries(accountsByInstitution).map(
          ([institution, institutionAccounts]) => {
            ...
            const isPlaidLinked =
              plaidItemId &&
              institutionAccounts.every((a) => a.plaid_item_id === plaidItemId);
            ...
            const hasError = institutionAccounts.some(
              (a) => a.last_error_code || a.needs_reauth,
            );
            const errorAccount = institutionAccounts.find(
              (a) => a.last_error_code || a.needs_reauth,
            );
                    ...
                    <Box flex={1}>
                      <HStack spacing={2} align="center">
                        ...
                        {hasError && (
                          <Tooltip
                            label={
                              errorAccount?.last_error_message ||
                              "Connection issue - check account status"
                            }
                          >
                    ...
                    <HStack spacing={2}>
                      {isPlaidLinked && (
                        <IconButton
                          ...
                          onClick={() =>
                            syncTransactionsMutation.mutate(plaidItemId)
                          }
                          isLoading={isSyncing}
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          toggleInstitutionVisibility(institutionAccounts)
                        }
                      >
                        {allVisible(
                          institutionAccounts.filter(canModifyAccount),
                        )
                          ? "Hide All"
                          : "Show All"}
                      </Button>
                      ...
                      <Tr>
                        <Th>
                          <Checkbox
                            ...
                            isIndeterminate={
                              institutionAccounts.some((a) =>
                                selectedAccounts.has(a.id),
                              ) &&
                              !institutionAccounts.every((a) =>
                                selectedAccounts.has(a.id),
                              )
                            }
                            onChange={(e) =>
                    ...
                    <Tbody>
                      {institutionAccounts.map((account) => (
                        ...
                        >
                          ...
                          <Td>
                            <Text fontSize="sm">
                              {formatAssetType(account.account_type)}
                            </Text>
                          </Td>
                          <Td isNumeric>
                            <Text
                            ...
                            <HStack spacing={2}>
                              <Tooltip
                                label={
                                  !canModifyAccount(account)
                                    ? "No permission to modify this account"
                                    : account.is_active
                                      ? "Hide account from everywhere"
                                      : "Show account everywhere"
                                }
                              >
                                <IconButton
                                  icon={
                              ...
                              >
                                <IconButton
                                  ...
                                  onClick={() =>
                                    navigate(
                                      `/accounts/${account.id}${selectedUserId ? `?user=${selectedUserId}` : ""}`,
                                    )
                                  }
                                />
                              </Tooltip>
        ...
        <AlertDialogOverlay>
          <AlertDialogContent>
            <AlertDialogHeader fontSize="lg" fontWeight="bold">
              Delete Account
              {deleteTarget === "selected" && selectedAccounts.size > 1
                ? "s"
                : ""}
            </AlertDialogHeader>
